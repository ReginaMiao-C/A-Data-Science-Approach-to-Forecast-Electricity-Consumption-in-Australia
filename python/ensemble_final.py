...
for SARIMAX_Weight in np.arange(0, 1.01, 0.01):
    for LSTM_Weight in np.arange(0, 1.01, 0.01):
        ...
        if AEMO_Weight < 0 or AEMO_Weight > 0.3:
            continue
...
# Make a peak value matrix across three models
value_scores = np.vstack([
    df_test_ensemble['sarimax_peak_value_predicted'],
    df_test_ensemble['lstm_peak_value_predicted'],
    df_test_ensemble['AEMO_peak_value_predicted']
])

# Calculate the time error
time_errors = np.vstack([
    (pd.to_datetime(df_test_ensemble['sarimax_peak_time_predicted']) -
     pd.to_datetime(df_test_ensemble['actual_peak_time'])).abs().dt.total_seconds() / 60,

    (pd.to_datetime(df_test_ensemble['lstm_peak_time_predicted']) -
     pd.to_datetime(df_test_ensemble['actual_peak_time'])).abs().dt.total_seconds() / 60,

    (pd.to_datetime(df_test_ensemble['AEMO_peak_time_predicted']) -
     pd.to_datetime(df_test_ensemble['actual_peak_time'])).abs().dt.total_seconds() / 60
])

# Normalize the value
value_norm = value_scores / value_scores.max(axis=0)
time_norm  = time_errors / time_errors.max(axis=0)

score = value_norm - time_norm

# Find the best model
best_model = np.argmax(score, axis=0)

df_test_ensemble['ensemble_peak_time_predicted'] = pd.to_datetime(
    np.choose(best_model, [
