...
# build background sequences for SHAP
def build_background_data(x, repeat_list, window_size, obs, max_samples=10):
    """
    x: feature dataframe
    repeat_list: repeat values used in rolling validation
    window_size: number of days in input window
    obs: number of observations per day
    max_samples: number of background samples for SHAP
    """
    background_list = []

    for repeat in repeat_list[:max_samples]:
        train_y_start = repeat
        train_x_start = repeat - window_size * obs
        train_x_end = train_y_start

        x_background = x.iloc[train_x_start:train_x_end]

        scaler_x = MinMaxScaler()
        x_background_scaled = scaler_x.fit_transform(x_background)
        x_background_scaled = torch.tensor(x_background_scaled, dtype=torch.float32).unsqueeze(0)

        background_list.append(x_background_scaled)

    return torch.cat(background_list, dim=0)
...
res_path = root_folder / 'Results' / 'LSTM' / 'Final'
res_path.mkdir(parents=True,exist_ok=True)
csv_name = 'train_val_results' + dropped_vars +'.csv'
plot_name = 'peak_results' + dropped_vars
results.to_csv(res_path / csv_name)

plt.figure(figsize=(12, 6))
sns.lineplot(data=results, x='date', y='true_peak', label='Actual', zorder=2)
sns.lineplot(data=results, x='date', y='pred_peak', label='Predicted', zorder=3)
for i in public_hol_list:
    plt.axvline(i, color='grey', ls='--', linewidth=0.5, zorder=1)
plt.legend()
...
# explain daily peak prediction rather than all 48 half-hour outputs
peak_model = PeakModel(model)
peak_model.eval()

# recreate repeat list used in rolling validation
repeat_list = list(range(70*obs, len(val_data), 40*obs))

# build background data from a small number of rolling windows
...
# explain the final validation sample processed in the loop
# x_val_scaled already exists from the last loop iteration
explain_data = x_val_scaled.clone()

# create SHAP explainer for differentiable PyTorch model
explainer = shap.GradientExplainer(peak_model, background_data)

# get SHAP values for explained sample
shap_values = explainer.shap_values(explain_data)

# handle output type if SHAP returns list
...
# aggregate SHAP values across time steps to get feature-level importance
shap_values_agg = np.mean(np.abs(shap_values), axis=1)
explain_data_agg = np.mean(explain_data_np, axis=1)
...
shap.summary_plot(
    shap_values_agg,
    features=explain_data_agg,
    feature_names=feature_names,
    show=False
)
plt.tight_layout()
plt.savefig(res_path / 'shap_summary_peak.png', bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(res_path / 'shap_bar_peak.png', bbox_inches='tight')
plt.close()

print('SHAP plots saved to: ', res_path)